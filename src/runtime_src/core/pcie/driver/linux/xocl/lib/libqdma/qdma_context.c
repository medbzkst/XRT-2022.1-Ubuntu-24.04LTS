		...
		struct mbox_msg_intr_ctxt *ictxt)
{
	int i = 0;
	int rv;
	//u32 intr_ctxt[QDMA_REG_IND_CTXT_WCNT_3];
	int ring_index;
	...
	for (i = 0; i < ictxt->num_rings; i++) {
		...
		rv = hw_indirect_ctext_prog(xdev, ring_index, QDMA_CTXT_CMD_CLR,
					QDMA_CTXT_SEL_COAL, NULL,
					QDMA_REG_IND_CTXT_WCNT_8, 0, 1);
		if (rv < 0)
			return rv;
		...
		rv = hw_indirect_ctext_prog(xdev, ring_index, QDMA_CTXT_CMD_RD,
					QDMA_CTXT_SEL_COAL, intr_ctxt,
					QDMA_REG_IND_CTXT_WCNT_3, 1, 1);
		if (rv < 0)
			return rv;
				...
				bool st, bool c2h, bool clr)
{
	...
	if (st && c2h) {
		rv = hw_indirect_ctext_prog(xdev, qid_hw, QDMA_CTXT_CMD_CLR,
					QDMA_CTXT_SEL_PFTCH, NULL, 0, 0, 1);
		if (rv < 0)
			return rv;
		rv = hw_indirect_ctext_prog(xdev, qid_hw, QDMA_CTXT_CMD_CLR,
					QDMA_CTXT_SEL_CMPT, NULL, 0, 0, 1);
		if (rv < 0)
			return rv;
	}
				...
				struct hw_descq_context *context)
{
	...
	rv = hw_indirect_ctext_prog(xdev, qid_hw, QDMA_CTXT_CMD_RD, sel,
				context->sw, QDMA_REG_IND_CTXT_WCNT_5, 0, 1);
	if (rv < 0)
		return rv;

	sel = c2h ? QDMA_CTXT_SEL_HW_C2H : QDMA_CTXT_SEL_HW_H2C;
	rv = hw_indirect_ctext_prog(xdev, qid_hw, QDMA_CTXT_CMD_RD, sel,
				context->hw, QDMA_REG_IND_CTXT_WCNT_2, 0, 1);
	if (rv < 0)
		return rv;

	sel = c2h ? QDMA_CTXT_SEL_CR_C2H : QDMA_CTXT_SEL_CR_H2C;
	rv = hw_indirect_ctext_prog(xdev, qid_hw, QDMA_CTXT_CMD_RD, sel,
				context->cr, QDMA_REG_IND_CTXT_WCNT_1, 0, 1);
	if (rv < 0)
		return rv;

	if (st && c2h) {
		rv = hw_indirect_ctext_prog(xdev, qid_hw, QDMA_CTXT_CMD_RD,
					QDMA_CTXT_SEL_CMPT, context->cmpt,
					QDMA_REG_IND_CTXT_WCNT_5, 0, 1);
		if (rv < 0)
			return rv;
		rv = hw_indirect_ctext_prog(xdev, qid_hw, QDMA_CTXT_CMD_RD,
					QDMA_CTXT_SEL_PFTCH, context->prefetch,
					QDMA_REG_IND_CTXT_WCNT_2, 0, 1);
		if (rv < 0)
			return rv;
	}
	...
	int ring_index, unsigned int ctxt_sz, u32 *context)
{
	...
	rv = hw_indirect_ctext_prog(xdev, ring_index, QDMA_CTXT_CMD_RD,
				QDMA_CTXT_SEL_COAL, context, ctxt_sz, 0, 1);
	if (rv < 0)
		return rv;
...

{
	...
	rv = hw_indirect_ctext_prog(xdev, qid_hw, QDMA_CTXT_CMD_WR, sel,
				context->sw, QDMA_REG_IND_CTXT_WCNT_5, 1, 1);
	if (rv < 0)
		return rv;
