...
struct drv_mode_name qdma4_mode_name_list[] = {
	{ AUTO_MODE,			"auto"},
	{ POLL_MODE,			"poll"},
			...
			struct qdma_descq *descq, struct qdma_request *req)
{
	...
	if (!cb->done || cb->status) {
		pr_err("%s: req 0x%p, %c,%u,%u/%u,0x%llx, done %d, err %d, tm %u.\n",
				...
				cb->status,
			req->timeout_ms);
		qdma4_descq_dump(descq, NULL, 0, 1);
		unlock_descq(descq);
			...
			struct qdma_descq *descq, struct qdma_request *req)
{
	...
	pr_debug("%s: req 0x%p, %u, sgl 0x%p,%u, wait %d, tm %u ms.\n",
		descq->conf.name, req, req->count, req->sgl, req->sgcnt, wait,
		req->timeout_ms);
 ...
 * @return	<0: error
 *****************************************************************************/
int qdma4_queue_get_config(unsigned long dev_hndl, unsigned long id,
		struct qdma_queue_conf *qconf,
		char *buf, int buflen)
{
	...
	descq = qdma4_device_get_descq_by_id(xdev, id, buf, buflen, 0);
 ...
 *
 *****************************************************************************/
int qdma4_device_capabilities_info(unsigned long dev_hndl,
...
int qdma4_config_reg_dump(unsigned long dev_hndl, char *buf,
		int buflen)
{
 ...
 * @return	<0: error
 *****************************************************************************/
int qdma4_queue_dump(unsigned long dev_hndl, unsigned long id, char *buf,
				int buflen)
{
	struct xlnx_dma_dev *xdev = (struct xlnx_dma_dev *)dev_hndl;
	struct qdma_descq *descq;
 ...
 * @return	<0: error
 *****************************************************************************/
int qdma4_queue_dump_desc(unsigned long dev_hndl, unsigned long id,
			...
			int buflen)
{
	...
	len = qdma4_descq_dump_state(descq, buf, buflen);
	if (descq->q_state != Q_STATE_ONLINE)
		return len;

	/** dump the queue descriptor state */
	len += qdma4_descq_dump_desc(descq, start, end, buf + len, buflen - len);
	return len;
}
 ...
 * @return	<0: error
 *****************************************************************************/
int qdma4_queue_dump_cmpt(unsigned long dev_hndl, unsigned long id,
			unsigned int start, unsigned int end, char *buf,
			int buflen)
{
	struct xlnx_dma_dev *xdev = (struct xlnx_dma_dev *)dev_hndl;
	 ...
	 */
	if (descq->q_state == Q_STATE_ONLINE)
		len += qdma4_descq_dump_cmpt(descq, start, end, buf + len,
				buflen - len);
 ...
 * @return	<0: error
 *****************************************************************************/
int qdma4_queue_remove(unsigned long dev_hndl, unsigned long id, char *buf,
			int buflen)
{
	struct xlnx_dma_dev *xdev = (struct xlnx_dma_dev *)dev_hndl;
	struct qdma_descq *descq;
 ...
 * @return	<0: error
 *****************************************************************************/
int qdma4_queue_config(unsigned long dev_hndl, unsigned long qid,
 ...
 * @return	<0: error
 *****************************************************************************/
int qdma4_queue_list(unsigned long dev_hndl, char *buf, int buflen)
{
	struct xlnx_dma_dev *xdev = (struct xlnx_dma_dev *)dev_hndl;
	struct qdma_dev *qdev;
	struct qdma_descq *descq = NULL;
	...
	if (h2c_qcnt > 0) {
		...
		for (i = 0; i < h2c_qcnt; i++, descq++) {
			...
			if (descq->q_state != Q_STATE_DISABLED)
				cur +=
				qdma4_descq_dump(descq, cur, end - cur, 0);
			unlock_descq(descq);
 ...
 * @return	<0: error
 *****************************************************************************/
int qdma4_queue_add(unsigned long dev_hndl, struct qdma_queue_conf *qconf,
			unsigned long *qhndl, char *buf, int buflen)
{
	...
	*qhndl = QDMA_QUEUE_IDX_INVALID;
	...
	rv = qdma4_device_prep_q_resource(xdev);
	if (rv < 0) {
#ifdef __QDMA_VF__
		lock_descq(descq);
		descq->q_state = Q_STATE_DISABLED;
...
#ifndef __QDMA_VF__
	if (xdev->conf.qdma_drv_mode == LEGACY_INTR_MODE) {
		rv = qdma4_intr_legacy_setup(descq);
		if (rv > 0) {
...
#endif
	/** fill in config. info */
	qdma4_descq_config(descq, qconf, 0);
#ifndef __QDMA_VF__
 ...
 * @return	<0: error
 *****************************************************************************/
int qdma4_queue_start(unsigned long dev_hndl, unsigned long id,
		     char *buf, int buflen)
{
	struct qdma_descq *descq;
	struct xlnx_dma_dev *xdev = (struct xlnx_dma_dev *)dev_hndl;
	...
	rv = qdma4_descq_config_complete(descq);
	if (rv < 0) {
...
clear_context:
	qdma4_descq_context_clear(descq->xdev, descq->qidx_hw,
					descq->conf.st, descq->conf.q_type, 1);
	qdma4_descq_free_resource(descq);

	return rv;
}

int qdma4_get_queue_state(unsigned long dev_hndl, unsigned long id,
 ...
 * @return	<0: error
 *****************************************************************************/
int qdma4_queue_stop(unsigned long dev_hndl, unsigned long id, char *buf,
			int buflen)
{
	struct xlnx_dma_dev *xdev = (struct xlnx_dma_dev *)dev_hndl;
	struct qdma_descq *descq;
	...
	qdma4_thread_remove_work(descq);
 ...
 * @return	<0: error
 *****************************************************************************/
int qdma4_get_queue_count(unsigned long dev_hndl,
 ...
 * @return	<0: error
 *****************************************************************************/
int qdma4_intr_ring_dump(unsigned long dev_hndl, unsigned int vector_idx,
	int start_idx, int end_idx, char *buf, int buflen)
{
	...
	if ((xdev->conf.qdma_drv_mode != INDIRECT_INTR_MODE) &&
		...
		pr_err("Interrupt aggregation not enabled in %s mode",
				qdma4_mode_name_list[xdev->conf.qdma_drv_mode].name);
		snprintf(buf, buflen,
...
handle_truncation:
  ...
  * @return  <0: error
  *****************************************************************************/
int qdma4_software_version_info(char *software_version)
{
	if (!software_version) {
	...
	sprintf(software_version, "%s", LIBQDMA4_VERSION_STR);
 ...
 * @return	<0:	error
 *****************************************************************************/
int libqdma4_init(unsigned int num_threads, void *debugfs_root)
{
	...
	ret = qdma4_threads_create(num_threads);
	if (ret < 0) {
		pr_err("qdma4_threads_create failed for num_thread=%d",
			       num_threads);
		return ret;
	}
#ifdef _QDMA4_DEBUGFS_
	...
	ret =  qdma4_debugfs_init(&qdma_debugfs_root);
	if (ret < 0) {
		pr_err("qdma4_debugfs_init failed for num_thread=%d",
				num_threads);
		return ret;
	}
#endif
