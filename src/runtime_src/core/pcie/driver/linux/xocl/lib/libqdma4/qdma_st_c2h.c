 ...
 *
 */
int qdma4_descq_st_c2h_read(struct qdma_descq *descq, struct qdma_request *req,
				...
				bool refill)
{
	...
	list_for_each_entry_safe(cb, tmp, &descq->pend_list, list) {
		...
		rv = qdma4_descq_st_c2h_read(descq, (struct qdma_request *)cb,
						update_pidx, refill);
		if (rv < 0) {
			pr_info("req 0x%p, error %d.\n", cb, rv);
			qdma4_sgt_req_done(descq, cb, rv);
			continue;
		}
...
int qdma4_descq_process_completion_st_c2h(struct qdma_descq *descq, int budget,
					bool upd_cmpl)
{
	struct xlnx_dma_dev *xdev = descq->xdev;
...
int qdma4_queue_c2h_peek(unsigned long dev_hndl, unsigned long id,
...
int qdma4_queue_packet_read(unsigned long dev_hndl, unsigned long id,
			struct qdma_request *req, struct qdma_cmpl_ctrl *cctrl)
{
	...
	lock_descq(descq);
	qdma4_descq_st_c2h_read(descq, req, 1, 1);
	unlock_descq(descq);
